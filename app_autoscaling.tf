...
resource "aws_appautoscaling_policy" "cpu_utilization" {
  ...
  target_tracking_scaling_policy_configuration {
    target_value = var.autoscaling_configuration.cpu_threshold_value
    # amount of time, in seconds, 
    # after a scale in or scale out activity has completed 
    # where further scaling activities are suspended
    scale_in_cooldown  = var.autoscaling_configuration.scale_in_cooldown
    scale_out_cooldown = var.autoscaling_configuration.scale_out_cooldown
...
resource "aws_appautoscaling_policy" "memory_utilization" {
  ...
  target_tracking_scaling_policy_configuration {
    target_value = var.autoscaling_configuration.memory_threshold_value
    # amount of time, in seconds, 
    # after a scale in or scale out activity has completed 
    # where further scaling activities are suspended
    scale_in_cooldown  = var.autoscaling_configuration.scale_in_cooldown
